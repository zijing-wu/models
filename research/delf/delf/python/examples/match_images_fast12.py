...
_REBUILD_TREE = True
_TREE_SAVE_FILE = 'annoy_tree_ds8_ds1_test.ann'
...
def main():
    ...
    if(_DEBUG):
        train_dir, test_dir, out_dir, loadtrain, loadtest = (
            'train_local_features_ds8', 'test_local_features_ds8', 'lines_out_test', 'n', 'n')
    ...
    batch_size = int(len(idx_arr_test)/_SAVE_NPZ_NUM)
    for j in range(_SAVE_NPZ_NUM):
        start_i = j*batch_size
        end_i = (j+1)*batch_size
        if(j==_SAVE_NPZ_NUM-1):
            end_i = len(idx_arr_test)
        with Pool(processes=_QUERY_PROCESSOR) as pool:
            data_lines = pool.starmap(f3, product(range(start_i, end_i)))

        out_file_path = "%s_%d.npz"%(out_dir, j)
        print("saving output...[%s]" % (out_file_path))
